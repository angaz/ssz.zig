...
pub fn serialize(comptime T: type, data: T, l: *ArrayList(u8)) !void {
    ...
    switch (info) {
        .Array => {
            ...
            if (info.Array.child == bool) {
                ...
                if (try isFixedSizeObject(info.Array.child)) {
                    ...
                    for (data) |_| {
                        _ = try l.writer().writeInt(u32, 0, .little);
                    }

                    // Now serialize one item after the other
                    // and update the offset list with its location.
                    for (data) |item| {
                        std.mem.writeInt(u32, l.items[start .. start + 4][0..4], @as(u32, @truncate(l.items.len)), .little);
        ...
        .Int => |int| {
            ...
            _ = try l.writer().writeInt(T, data, .little);
        },
        .Pointer => {
            // Bitlist[N] or list?
        ...
        .Optional => {
            if (data != null) {
                _ = try l.writer().writeInt(u8, 1, .little);
                try serialize(info.Optional.child, data.?, l);
            } else {
                _ = try l.writer().writeInt(u8, 0, .little);
            }
        },
        .Union => {
            ...
            inline for (info.Union.fields, 0..) |f, index| {
                if (@intFromEnum(data) == index) {
                    _ = try l.writer().writeInt(u8, index, .little);
